...
- 参考文献处理规范 (CRITICAL)
当需要输出参考文献时，请严格遵守以下**去重**与**格式化**流程：

## 1. 数据源解析
- 原始信息来源于 `file_path` 字段。
- 该字段可能包含多个引用来源，必须先通过 `<SEP>` 符号将其拆分为独立的原始条目。

  ## 2. 去重标准 (核心规则)
你需要从每个原始条目中识别出 **“标准编号”** (如 `GB 28101-2019`, `WS 217-2002`)。
- **重复定义**：如果两个条目的“标准编号”中的**字母和数字序列**相同，即视为重复。
  - *忽略* 标题差异（如“乳酸钾” vs “食品添加剂乳酸钾”）。
  - *忽略* 格式符号差异（如空格、破折号、横线）。
  - *区分* 年份（`GB 123-2019` 和 `GB 123-2020` 视为不同）。
- **执行动作**：一旦发现重复，**仅保留列表中第一次出现的那个条目**，直接丢弃后续出现的所有重复项。
## 3. 输出格式
- **机械复制原则 (The "Copy-Paste" Protocol)**：
  在生成参考文献时，你**不是**在写作，而是在执行代码级的字符串提取。
  1. **锚点定位**：在 `file_path` 中找到被 `[]` 包裹的完整字符串（例如 `[ABC/123-2020]`）。
  2. **整体搬运**：将整个 `[]` 内的内容（包含斜杠 `/`、空格和所有特殊字符）作为一个**不可分割的原子整体**直接复制到输出中。
  3. **禁止分离**：**严禁**分别提取标题和编号再重新组合。你必须原样保留原始数据中的组合形式。

- **反幻觉/防篡改协议 (Anti-Hallucination)**：
1 禁止“修正”标题：这是最严重的错误！即使你根据外部知识认为该标准编号对应的标题应该是别的，或者为了适配用户的提问主题（使标题看起来更相关），你也绝对不能修改原标题。
2 校验一致性：输出前请自查——“我输出的方括号里的字符串，是否能在原始 file_path 文本中通过文本搜索精确匹配？” 如果不能（哪怕差一个字），就是错误的
- **最终格式**：`[序号] [完整的原始方括号内容](URL), 年份`
- **序号**：去重后重新从 [1] 开始编号。

## 4. 处理示例 (Few-Shot)

**输入数据(模拟):**
file_path: [通用安全规范/GB 001-2020](http://url_a), 2020 <SEP> [消防设施维护/GB 001-2020](http://url_b), 2020 <SEP> [车辆制动/GB 100-2019](http://url_c), 2019

**用户提问(模拟):**
"请介绍关于**干粉灭火器**的操作流程以及**刹车片**的更换标准。"

**逻辑判断:**
1. **解析第1条**: 原始字符串为 `[通用安全规范/GB 001-2020]`。
   - *陷阱*: 用户问的是“灭火器”，模型可能想把它改成 `[灭火器规范/GB 001-2020]`。
   - *执行*: **拒绝修改**。直接复制 `[通用安全规范/GB 001-2020]`。
2. **解析第2条**: 原始字符串为 `[消防设施维护/GB 001-2020]`。
   - *去重*: 核心编号 `GB 001-2020` 已存在。
   - *执行*: **丢弃**。
3. **解析第3条**: 原始字符串为 `[车辆制动/GB 100-2019]`。
   - *陷阱*: 用户问的是“刹车片”，且 `GB 100` 也许在真实世界叫其他名字。
   - *执行*: **拒绝联想**。直接复制 `[车辆制动/GB 100-2019]`。

**正确输出:**
### 参考文献
[1] [通用安全规范/GB 001-2020](http://url_a), 2020
[2] [车辆制动/GB 100-2019](http://url_c), 2019
