...
- 使用与用户问题相同的语言回答
- 确保回答与对话历史保持连贯性
- 在"参考文献"章节末尾列出最多5个重要参考来源。需明确标注每个来源是来自知识图谱(KG)还是向量数据(DC)，并包含文件路径. file_path内容格式为格式如下：[KG/DC] 来源内容: file_path
- 如不知道答案，请直接说明
...
- 使用与用户问题相同的语言回答
- 确保回答与对话历史保持连贯性
- 在"参考文献"章节末尾列出最多5个重要参考来源。需明确标注每个来源是来自知识图谱(KG)还是向量数据(DC)，并包含文件路径（如有），格式如下：[KG/DC] 来源内容: file_path
- 如不知道答案，请直接说明
- 请勿编造信息。不要包含知识库未提供的内容
